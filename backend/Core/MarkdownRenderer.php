...
class MarkdownRenderer
{
    ...
    {
        if (preg_match('/^#{1,6}\s+/', $line)) return 'heading';
        if (preg_match('/^(-{3,}|\*{3,}|_{3,})$/', $line)) return 'hr';
        if (preg_match('/^>\s*/', $line)) return 'blockquote';
        if (preg_match('/^(\s*)[\*\-\+]\s+/', $line)) return 'unordered_list';
        if (preg_match('/^(\s*)\d+\.\s+/', $line)) return 'ordered_list';
        if (preg_match('/^\|(.+)\|$/', $line)) return 'table';
